...
type errPayloadGame struct {
	Error   string                 `json:"error"`
	Message string                 `json:"message"`
	Details map[string]interface{} `json:"details,omitempty"`
}

func TestSubscribeGame_UnauthorizedNoCookie_401(t *testing.T) {
	cfg := testutil.NewTestConfig()
	log := testutil.NewTestLogger()
	srv := httptest.NewServer(testutil.NewTestRouter(cfg, log, "http://127.0.0.1:9999"))
	t.Cleanup(srv.Close)

	resp, err := http.Get(srv.URL + "/events/game/gam_xyz789")
	if err != nil {
...
func TestSubscribeGame_AuthorizedMember_HeadersAndHeartbeat(t *testing.T) {
	// Mock APIGateway with /internal/validate and /games/{id}
	...
	cfg := testutil.NewTestConfig()
	cfg.InternalToken = "test-internal"
	log := testutil.NewTestLogger()
	srv := httptest.NewServer(testutil.NewTestRouter(cfg, log, gw.URL))
	t.Cleanup(srv.Close)

	req, _ := http.NewRequest(http.MethodGet, srv.URL+"/events/game/gam_xyz789", nil)
	...
	// Read SSE stream and assert keep_alive within ~300ms (interval=100ms)
