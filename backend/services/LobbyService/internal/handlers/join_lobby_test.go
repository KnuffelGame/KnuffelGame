...
func TestJoinLobby_KickedUserCanRejoin(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock DB: %v", err)
	}
	...
	// Get player count (currently 1 player in lobby, room for more)
	mock.ExpectQuery("SELECT COUNT").
		WithArgs(lobbyID).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(1))

	// Check if user is already a member (false, since previously kicked user has is_active=false)
	mock.ExpectQuery("SELECT EXISTS").
		WithArgs(lobbyID, userID).
		WillReturnRows(sqlmock.NewRows([]string{"exists"}).AddRow(false))
